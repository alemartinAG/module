...
static int __init encrypt_init(void){
   ...
   printk(KERN_INFO "Encrypt: Inicializando el modulo de encriptacion\n");
   ...
   printk(KERN_INFO "Encrypt: dispositivo registrado correctamente con major number %d\n", majorNumber);
...
static void __exit encrypt_exit(void){
   ...
   printk(KERN_INFO "Encrypt: dispositivo eliminado!\n");
}
 
//Se llama cada vez que se abre el dispositivo
static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "Encrypt: El dispositivo ha sido abierto %d veces\n", numberOpens);
   return 0;
}
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   if (error_count==0){            // Si no hay errores
      printk(KERN_INFO "Encrypt: Se enviaron %d caracteres al usuario\n", size_of_message);
      return (size_of_message = 0);  // clear the position to the start and return 0
   }
   else {
      printk(KERN_INFO "Encrypt: Fallo al enviar %d caracteres al usuario\n", error_count);
